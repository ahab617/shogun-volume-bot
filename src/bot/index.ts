...
async function loadCommands() {
	...
	bot.setMyCommands(commands).then(res => {
		console.log(`Register bot menu commands${res ? "success" : "fail"} ${commands.length}个`)
	}).catch(err => {
		console.log("The menu command for registering bot is wrong", err.message)
	})
}

export function getName(chat_member: any) {
...
export function delMsg(chat_id: any, msg_id: any, time: any) {
	setTimeout(() => {
...
export class Commands {
	constructor(reg: RegExp, descript: string, cmd: string, isCommands: boolean, fn: Function, cb: Function) {
		this.reg = reg
